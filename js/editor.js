...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  editorDivs.forEach((div) => {
    const rawInitial = div.getAttribute("data-initial-code") || "print('Hallo!')";
    const initialCode = rawInitial.replace(/\\n/g, "\n");
    let tests = [];
    const rawTests = div.getAttribute("data-tests");
    if (rawTests) {
      try { tests = JSON.parse(rawTests); } catch {}
    }
    ...
    let runButton = div.nextElementSibling;
    while (runButton && !runButton.classList.contains("python-run")) runButton = runButton.nextElementSibling;

    let output = runButton ? runButton.nextElementSibling : null;
    while (output && !output.classList.contains("python-output")) output = output.nextElementSibling;

    let checkButton = runButton ? runButton.nextElementSibling : null;
    while (checkButton && !checkButton.classList.contains("python-check")) checkButton = checkButton.nextElementSibling;

    let feedback = checkButton ? checkButton.nextElementSibling : null;
    while (feedback && !feedback.classList.contains("python-feedback")) feedback = feedback.nextElementSibling;
  ...
  instances.forEach(({ editor, checkButton, feedback, tests }) => {
    ...
    checkButton.addEventListener("click", async () => {
      if (!tests.length) { feedback.textContent = "Geen tests."; return; }
